...
echo $NOW
STOCKDATE=$(date --date='-1 month' +"%Y%m")
STOCKFOLDER="sirene"'_'${STOCKDATE}'_'"L_M"
chmod -Rf 777 /home/igeo/partage/sirene/data/data.gouv.fr/stock/
cd /home/igeo/partage/sirene/data/data.gouv.fr/stock/
rm -Rf $STOCKFOLDER && mkdir $STOCKFOLDER && cd $STOCKFOLDER
#ON SE PLACE DANS LE DOSSIER DE TELECHARGEMENT ET ON TELECHARGE LE FICHIER DE MAJ MENSUELLE
wget http://files.data.gouv.fr/sirene/$STOCKFOLDER.zip
unzip $STOCKFOLDER".zip"
rm -Rf $STOCKFOLDER".zip"
STOCKFILE=${STOCKFOLDER}".csv"
mv *.csv $STOCKFILE

echo "2/8 : DECOUPAGE DU FICHIER NATIONAL EN FICHIERS DEPARTEMENTAUX"
#for d in `seq -w 1 19` 2A 2B `seq 21 99`; do
for d in 38 69; do
  head -n 1 $STOCKFILE > sirene_$d.csv
done
# split du fichier national sur DEPET (25ème colonne)
#awk -F";" -v OFS=";" 'BEGIN { FPAT = "([^;]+)|(\"[^\"]+\")" } {gsub("\"","",$25); print >> "sirene_"$25".csv" }' $STOCKFILE
awk -F";" -v OFS=";"  'BEGIN { FPAT = "([^;]+)|(\"[^\"]+\")" } {gsub("\"","",$25); if($25 == '38' || $25 == '69') print >> "sirene_"$25".csv" }' $STOCKFILE
...
chmod -Rf 777 cd /home/igeo/partage/sirene/data/data.gouv.fr/stock/$STOCKFOLDER/
MAJTABLE="sirene.vca_stock_sirene_"$STOCKDATE
INPUTSTOCKFOLDER="/home/igeo/partage/sirene/data/data.gouv.fr/stock/$STOCKFOLDER/sirene_vca.csv"
OUTPUTCSVFILE="/home/igeo/partage/sirene/data/data.gouv.fr/stock/$STOCKFOLDER/EUDONET_STOCK"'_'$STOCKFOLDER".csv"
chmod -Rf 777 /home/igeo/partage/sirene/data/data.gouv.fr/stock/$STOCKFOLDER
sudo -u postgres psql -d vca -c "SET client_encoding = 'utf-8'"
sudo -u postgres psql -d vca -v table=$MAJTABLE -v folder="'$INPUTSTOCKFOLDER'" -f "/home/igeo/partage/sirene/sql/01-CREATE_vca_stock_sirene.sql"
sudo -u postgres psql -d vca -v table=$MAJTABLE -v output_csvfile="'$OUTPUTCSVFILE'" -f "/home/igeo/partage/sirene/sql/02-EXPORT_vca_stock_sirene_TO_Eudonet.sql"
unoconv --format xls -i FilterOptions="59,34,0,1,1/2/13/2/87/5,1036,true,true" "EUDONET_STOCK"'_'$STOCKFOLDER".csv"

#echo -ne "Bonjour,\nVoici la pièce jointe.\nCordialement" | mutt "fallamanche@vienne-condrieu-agglomeration.fr" -b "igeo@vienne-condrieu-agglomeration.fr" -s "EXPORT MENSUEL - SIRENE" -a "EUDONET_STOCK"'_'$STOCKFOLDER".xls"